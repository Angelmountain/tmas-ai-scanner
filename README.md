...
Tests any OpenAI-compatible LLM endpoint for vulnerabilities including prompt injection, jailbreaking, system prompt leakage, data exfiltration, and more — using OWASP LLM Top 10 and MITRE ATT&CK frameworks.
...
# Generate config (references env var name, not the key itself)
...
# Run scan (set both Vision One and LLM API keys as env vars)
...
target:
  name: "gpt-4"
  endpoint: "https://api.openai.com/v1"
  api_key_env: "LLM_API_KEY"  # name of env var containing the LLM API key
  model: "gpt-4"
  system_prompt: "You are a helpful assistant"  # optional
attack_preset: "owasp"  # or "mitre"
```

**Important:** TMAS uses `api_key_env` to reference the **name of an environment variable** that contains the API key, not the key itself. The actual key must be set in the environment (e.g. `export LLM_API_KEY=sk-...`).
